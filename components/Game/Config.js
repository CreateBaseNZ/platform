...
import themes, { themeFiles } from "../../constants/themes";
...
const Config = (props) => {
	...
	const themeClickHandler = (theme) => {
		localStorage.setItem("createbase__monaco-theme", theme.replace(/([\ +'])/g, ""));
				...
				{themes[category].map((t) => {
					...
					return (
						<div
							...
							onMouseEnter={mouseEnterHandler.bind(this, t, themeFiles[themeName].colors["editor.foreground"], themeFiles[themeName].colors["editor.background"])}
	...
	return (
		<div className={`${classes.config} ${props.show ? "" : "hide"}`}>
			<div className={classes.configTabContainer}>
				{tabs.map((tab) => (
					<div key={tab} className={`${classes.configTab} ${activeTab === tab ? classes.activeTab : ""}`} onClick={tabClickHandler.bind(this, tab)}>
						{tab}
					</div>
				))}
			</div>
			<div className={classes.themeSelect} style={{ display: activeTab !== "Theme" && "none" }}>
				<div className={classes.configEditorWrapper}>
					<Editor language="javascript" value={lorem} onMount={editorDidMount} options={editorOptions} />
				</div>
				{themeOptions("Light Themes")}
				{themeOptions("Dark Themes")}
				{themeOptions("Funky Themes")}
			</div>
			<div className={classes.message} style={{ display: activeTab !== "Message Us" && "none" }}>
				<h4>We want to hear from you</h4>
				<p>
					Your feedback will help us deliver the best experience to creators like yourself. Whether you've found a bug, got some feedback, just want to have a chat, or anything else - get in touch!
					We'd love to hear your thoughts.
				</p>
				<a href="https://forms.gle/VJNUzpAhXG4KtiZz6" target="_blank" title="Message Us - Google Forms">
