...
const NodeBoolean = memo(
  ({ data = { connections: [] }, isConnectable, label }) => {
    return (
      <div
        className={`${classes.node} ${classes.utils} ${classes.hasRightHandle} ${classes.nodeBoolean}`}
      >
        <h4>{label}</h4>
        <CustomHandle
          type="source"
...
export const NodeFalse = memo(({ data, isConnectable }) => {
  return (
    <NodeBoolean data={data} isConnectable={isConnectable} label="FALSE" />
  );
});
...
export const NodeAbsolute = memo(
  ({ id, data = { values: { a: 1 }, connections: [] }, isConnectable }) => {
    return (
      <div
        className={`${classes.node} ${classes.operating} ${classes.hasLeftHandle} ${classes.hasRightHandle}`}
      >
        <CustomHandle
          type="target"
          position="top"
