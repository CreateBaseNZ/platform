...
const Header = ({ session, type, org, name = "" }) => {
	...
	return (
		<header className={classes.header}>
			...
			<Link href="/browse">
				<TertiaryButton className={classes.browse} title="Browse projects" mainLabel="Browse projects" />
			</Link>
			{session ? (
				type && (
					<div className={classes.avatar} tabIndex={-1} onBlur={() => setActive(false)}>
						<div className={`${classes.icon} ${classes[type]} ${active ? classes.active : ""}`} onClick={() => setActive((state) => !state)}>
							<i className="material-icons-outlined">{type === "admin" ? "verified_user" : type === "educator" ? "school" : "backpack"}</i>
						</div>
