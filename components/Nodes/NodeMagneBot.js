...
export const NodeMagnebotMoveArm = memo(({ id, data = { values: getDefaultValues("NodeMagnebotMoveArm"), connections: [] }, isConnectable }) => {
	return (
		<div className={`${classes.node} ${classes.actioning} ${classes.nodeMoveArm} ${classes.hasLeftHandle} ${classes.hasRightHandle}`}>
			...
			<CustomHandle type="target" position="bottom" id="float__in__z" style={{ left: "auto", right: "36px", transform: "none" }} isConnectable={isConnectable} connections={data.connections} />
			<div className={classes.flexRow}>
				<div className={classes.flexCol}>
					<div className={classes.label}>X</div>
					<InputWithHandle data={data} blockId={id} handleId="float__in__x" inputName="x" />
				</div>
				<div className={classes.flexCol}>
					<div className={classes.label}>Y</div>
					<InputWithHandle data={data} blockId={id} handleId="float__in__y" inputName="y" />
				</div>
				<div className={classes.flexCol}>
