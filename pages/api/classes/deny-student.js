...
export default async function (req, res) {
	...
	try {
		data = (
			await axios.post(process.env.PREFIX_BACKEND + "/class/remove-member", {
				API_KEY_PRIVATE: process.env.API_KEY_PRIVATE,
				input: { query: { class: { _id: input.classId }, license: { _id: input.licenseId } }, date: input.date, status: "requested" },
			})
		)["data"];
	} catch (error) {
