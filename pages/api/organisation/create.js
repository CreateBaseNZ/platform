...
function validateCity(city) {
	if (!city) {
		return { status: false, content: "Please provide a city" };
	}
	return { status: true, content: "" };
}
...
export default async function (req, res) {
	...
	let data;
	try {
		data = (await axios.post("https://createbase.co.nz/organisation/create", { PRIVATE_API_KEY: process.env.PRIVATE_API_KEY, input }))["data"];
	} catch (error) {
		if (error.response) {
