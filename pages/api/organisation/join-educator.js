...
export default async function (req, res) {
	...
	if (session.user.organisation) {
		return res.send({ status: "critical error", content: "This user is already in an organisation" });
	}
	...
	let data;
	try {
		data = (await axios.post(process.env.ROUTE_URL + "/organisation/educator-join", { PRIVATE_API_KEY: process.env.PRIVATE_API_KEY, input }))["data"];
	} catch (error) {
